    ...
    <flux:sidebar-header>
        ...
        <x-slot name="button">
            <div class="flex gap-2">
                ...
                <flux:button :href="route('visits.create')" wire:navigate icon="plus" variant="primary"
                    size="sm">Tambah Pengiriman</flux:button>
            </div>
        </x-slot>
    </flux:sidebar-header>
    <flux:sidebar-content>
        <div class="flex min-w-4xl flex-wrap items-end gap-4">
            <div class="w-full sm:w-64">
                <flux:input wire:model.live.debounce.300ms="visitNo" label="Nomor Pengiriman" />
            </div>
            <div class="w-full sm:w-64">
                <flux:input wire:model.live.debounce.300ms="location" label="Outlet" />
            </div>
        ...
        @forelse ($this->visits as $index => $visit)
            <div wire:key="visit-{{ $visit->id }}"
                ...
                <div class="w-1/4 font-semibold text-mine-300 dark:text-mine-100">{{ $visit->visit_no }}</div>
                <div class="w-1/5">{{ $visit->visit_date->format('d/m/Y') }}</div>
                <div class="w-1/4">{{ $visit->location->name }}</div>
                <div class="w-1/6 text-center">{{ $visit->details_count }}</div>
                <div class="w-1/5">{{ $visit->creator->name }}</div>
                <div class="flex w-40 justify-center gap-1">
                    <flux:button :href="route('visits.show', $visit)" wire:navigate icon="eye" variant="ghost"
                        size="sm">Lihat</flux:button>
                    @if ($visit->isEditable())
                        <flux:button :href="route('visits.edit', $visit)" wire:navigate icon="pencil-square"
        ...
        @empty
            <div
                class="min-w-4xl border-t border-mine-200 py-10 text-center text-sm text-mine-300 dark:border-mine-400">
                Data pengiriman belum tersedia.</div>
        @endforelse
    </flux:sidebar-content>
</div>
