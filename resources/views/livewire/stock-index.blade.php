            ...
            <div class="w-full sm:w-64">
                <flux:select wire:model.live="productSlug" label="Produk">
                    ...
                    @foreach ($this->products as $product)
                        <flux:select.option wire:key="stock-filter-product-{{ $product->id }}"
                            value="{{ $product->slug }}">
            ...
            <div class="w-full sm:w-64">
                <flux:select wire:model.live="locationSlug" label="Lokasi">
                    ...
                    @foreach ($this->locations as $location)
                        <flux:select.option wire:key="stock-filter-location-{{ $location->id }}"
                            value="{{ $location->slug }}">
                            {{ $location->name }}
                        </flux:select.option>
                    @endforeach
            ...
            <table class="min-w-6xl w-full border-collapse text-sm">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" class="min-w-52 border border-mine-200 p-3 text-left dark:border-mine-400">
                            Lokasi</th>
                        <th rowspan="2" class="min-w-64 border border-mine-200 p-3 text-left dark:border-mine-400">
                            Produk</th>
                        <th colspan="5" class="border border-mine-200 p-3 text-center dark:border-mine-400">Rangkuman
                            Stok</th>
                    </tr>
                    <tr>
                        ...
                        <th
                            ...
                            Fisik</th>
                        <th class="min-w-28 border border-mine-200 p-2 text-right text-green-600 dark:border-mine-400">
                            Terjual</th>
                ...
                <tbody>
                    @forelse ($this->stocks as $index => $stock)
                        <tr wire:key="stock-{{ $stock['product_id'] }}-{{ $stock['location_id'] }}">
                            <td class="border border-mine-200 p-3 text-center dark:border-mine-400">{{ $index + 1 }}
                            </td>
                            <td class="border border-mine-200 p-3 dark:border-mine-400">
                                <div class="font-semibold">{{ $stock['location_name'] }}</div>
                                <div class="text-xs text-zinc-500">
                                    {{ $this->locationTypeLabel($stock['location_type']) }}</div>
                            </td>
                            <td class="border border-mine-200 p-3 dark:border-mine-400">
                                <div class="font-semibold text-mine-300 dark:text-mine-100">
                                    {{ $stock['product_name'] }}</div>
                            ...
                            <td class="border border-mine-200 p-3 text-right text-green-600 dark:border-mine-400">
                                {{ number_format($stock['sales']) }} Pcs</td>
                            <td class="border border-mine-200 p-3 text-right font-bold dark:border-mine-400">
                                {{ number_format($stock['total']) }} Pcs</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9"
                                class="border border-mine-200 p-10 text-center text-mine-300 dark:border-mine-400 dark:text-mine-100">
