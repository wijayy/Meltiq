...
<div class="flex h-[calc(100dvh-8rem)] flex-col gap-4 overflow-hidden lg:h-[calc(100dvh-4rem)]">
    ...
    <flux:sidebar-content>
            ...
            <div class="w-full sm:w-64">
                <flux:select wire:model.live="productSlug" label="Produk">
                    ...
                    @foreach ($this->products as $product)
                        <flux:select.option wire:key="movement-product-{{ $product->id }}"
                            value="{{ $product->slug }}">
            ...
            <div class="w-full sm:w-64">
                <flux:select wire:model.live="locationSlug" label="Lokasi">
                    ...
                    @foreach ($this->locations as $location)
                        <flux:select.option wire:key="movement-location-{{ $location->id }}"
                            value="{{ $location->slug }}">
                            {{ $location->name }}
                        </flux:select.option>
                    @endforeach
        ...
        <div class="flex min-w-6xl items-center gap-4 text-sm font-semibold">
            <div class="w-10">#</div>
            <div class="w-36">Tanggal</div>
            <div class="w-64">Produk</div>
            <div class="w-28 text-center">Tipe</div>
            <div class="w-48">Dari</div>
            <div class="w-48">Ke</div>
            <div class="w-28 text-right">Qty</div>
            <div class="w-40">Referensi</div>
        </div>
            ...
            <div wire:key="movement-{{ $movement->id }}"
                ...
                <div class="w-40">
                    @if ($movement->referenceUrl())
                        <flux:link :href="$movement->referenceUrl()" wire:navigate>
                            {{ $movement->reference_no ?? 'Lihat Detail' }}</flux:link>
                    @else
        ...
        <div class="pt-6">
            ...
            <flux:text class="mt-1">Stok bertambah dan berkurang ditampilkan terpisah untuk setiap product dan
                location.</flux:text>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full border-collapse text-sm">
                <thead>
                    <tr>
                        <th rowspan="2"
                            class="sticky left-0 min-w-64 border border-mine-200 bg-white p-3 text-left dark:border-mine-400 dark:bg-neutral-700">
                            Produk</th>
                        @foreach ($this->summaryLocations as $location)
                            <th colspan="{{ $location->type === 'virtual' ? 1 : 2 }}"
                                class="{{ $location->type === 'virtual' ? 'min-w-32' : 'min-w-64' }} border border-mine-200 p-3 text-center dark:border-mine-400">
                                {{ $location->name }}</th>
                        @endforeach
                    </tr>
                    <tr>
                        @foreach ($this->summaryLocations as $location)
                            <th
                                class="min-w-32 border border-mine-200 p-2 text-center text-green-600 dark:border-mine-400">
                                Bertambah</th>
                            @if ($location->type !== 'virtual')
                                <th
                                    class="min-w-32 border border-mine-200 p-2 text-center text-red-600 dark:border-mine-400">
                                    Berkurang</th>
                            @endif
                        @endforeach
                    </tr>
                ...
                <tbody>
                    @forelse ($this->summaryProducts as $product)
                        <tr wire:key="summary-product-{{ $product->id }}">
                            <td
                                class="sticky left-0 border border-mine-200 bg-white p-3 dark:border-mine-400 dark:bg-neutral-700">
                            ...
                            @foreach ($this->summaryLocations as $location)
                                ...
                                <td
                                    class="border border-mine-200 p-3 text-right font-semibold text-green-600 dark:border-mine-400">
                                    {{ number_format($movementSummary['increase']) }} Pcs</td>
                                @if ($location->type !== 'virtual')
                                    <td
                                        class="border border-mine-200 p-3 text-right font-semibold text-red-600 dark:border-mine-400">
                                        {{ number_format($movementSummary['decrease']) }} Pcs</td>
                                @endif
                            @endforeach
                        </tr>
                    @empty
                        <tr>
                            <td colspan="100%"
                                class="border border-mine-200 p-8 text-center text-mine-300 dark:border-mine-400">Belum
                                ada data untuk dirangkum.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
